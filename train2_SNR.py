...
class Trainer:
    ...
    def evaluate_SNR(self, data_loader):
        """Evaluate model
        ...
        with torch.no_grad():
            for x, y in data_loader:
                ...
                x_np = x.cpu().numpy()  # Convert to numpy for noise addition
                noisy_x_np = self.add_noise(x_np,1)
                noisy_x = torch.tensor(noisy_x_np, dtype=torch.float32).to(self.device)  # Convert back to tensor


                preds, recons = self.model(noisy_x)
