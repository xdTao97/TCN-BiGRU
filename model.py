...
class TCNSENetBiGRUGlobalAttention(nn.Module):
    def __init__(self, window_size, batch_size, input_dim, output_dim, num_channels, kernel_size,
        ...
        self.fc = nn.Linear(num_channels[-1] + hidden_layer_sizes[- 1] * 2, output_dim)


        self.recon_model = ReconstructionModel(window_size, num_channels[-1] + hidden_layer_sizes[- 1] * 2,
                                               num_channels[-1] + hidden_layer_sizes[- 1] * 2, output_dim,
                                               recon_n_layers,
                                               dropout)
