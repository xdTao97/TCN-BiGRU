...
class MTAD_GAT(nn.Module):
    def __init__(self, feats):
        ...
        self.feature_gat = FeatureAttentionLayer(feats, 1, feats)
        self.time_gat = TemporalAttentionLayer(feats, 1, feats)
    ...
    def forward(self, data, hidden):
        ...
        feat_r = self.feature_gat(data_r)
        data_t = torch.cat((torch.zeros(1, self.n_feats), data.t()))
        time_r = self.time_gat(data_t)
...
class GDN(nn.Module):
    def __init__(self, feats):
        ...
        # self.g = dgl.add_self_loop(self.g)
        self.feature_gat = FeatureAttentionLayer(1, 1, feats)
