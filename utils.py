...
def create_data_loaders(train_dataset, batch_size, val_split=0.1, shuffle=True, test_dataset=None):
    ...
    if val_split == 0.0:
        print(f"1train_size: {len(train_dataset)}")
        ...
        print(f"2.1train_size: {len(train_indices)}")
        print(f"validation_size: {len(val_indices)}")

    if test_dataset is not None:
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
        print(f"3test_size: {len(test_dataset)}")
...
def plot_losses(losses, save_path="", plot=True):
    ...
    #print("losses[""train_recon""]：", losses["train_recon"])
